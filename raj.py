    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            if user_id in bgmi_cooldown:
                ...
                if time_since_last_attack < 3:
                    remaining_time = 3 - time_since_last_attack
                    response = f"You are on cooldown. Please wait {remaining_time} seconds before running the /rajput command again."
                    bot.reply_to(message, response)
                return
...
def show_help(message):
 ...
 /rajput : Method For Bgmi Servers. 
